# erpilot Small Business License (SBL) v1.0

> **中文 / English bilingual** — both versions are equally authoritative.
> Last updated: 2026-05-16
>
> This is **one of three** license tracks for erpilot:
...
**If your organization has ≤ 20 concurrent erpilot users AND meets the
exclusion criteria below, you may use erpilot — including all closed-source
connectors — for free, without AGPL-3.0 source disclosure obligations.**

**只要你公司同時在線 erpilot 用戶 ≤ 20 人，且符合下列排除條件，你可以
完全免費使用 erpilot 全套功能（含閉源 connector），且不必依 AGPL-3.0 揭露改動 source。**

---

## 1. Eligibility / 適用條件

To use erpilot under this Small Business License, **ALL** of the following
must be true:
...
The **peak concurrent active users** of your erpilot deployment, measured
...
你的 erpilot 部署在**任一連續 24 小時內的同時在線活躍用戶峰值**不得超過 **20 人**。
...
The erpilot deployment must be used **by and within a single legal
...
erpilot 部署必須由**單一法人**（一家公司、一個政府機關、一所學校等）
**內部使用**。跨法人使用——即使同一企業集團——**不適用**本授權。

### 1.3 No External Service / 不對外提供服務

You must NOT use erpilot to provide a service to parties outside your
legal entity, including (but not limited to):

你**不得**用 erpilot 對你的法人以外提供服務，包含但不限於：
...
You must NOT embed erpilot (in whole or in part) into any product,
service, or system that is sold, licensed, distributed, or otherwise
provided to third parties.

你**不得**將 erpilot（全部或部分）嵌入任何銷售 / 授權 / 散布 / 提供給
第三方的產品、服務、或系統。
...
may deploy erpilot for individual clients ONLY when **each such client
independently qualifies under this license**. You may NOT distribute
modifications of erpilot to multiple clients without commercial licensing.

如果你是 SI / IT 顧問 / 服務商，你可以為個別客戶部署 erpilot，但**每個
客戶**必須**獨立符合**本授權條件。你**不得**未取得商業授權即把 erpilot
的修改版散布給多個客戶。
...
- (a) **Use** erpilot for any internal business purpose, including in
       production;
       將 erpilot 用於任何內部商業目的（含 production 環境）；
...
- (e) **Keep modifications private** — You are NOT obligated to disclose
       ...
       users access erpilot over a network. The AGPL-3.0 §13 ("Network
       use is distribution") clause is **explicitly waived** under this
       license.
       **保留修改不公開** — 你**沒有**揭露改動 / 衍生作品 / 設定的義務，
       即使使用者透過網路存取 erpilot。**AGPL-3.0 §13**「網路使用即散布」
       條款於本授權**明示豁免**。
...
You must retain a visible "Powered by erpilot Community" attribution in
at least one user-facing location (e.g. application footer, "About"
dialog, or settings page). You may not represent erpilot as Your own
product.

你必須在至少一處使用者可見處（如：應用程式 footer / "About" 對話框 /
設定頁）保留可見的 "Powered by erpilot Community" 署名。你**不得**把
erpilot 表現為你自己的產品。

### 3.2 No Sublicense / 不得再授權

You may NOT sublicense erpilot to third parties. Each end-user
organization must independently obtain its own license (AGPL-3.0, Small
Business, or Commercial).

你**不得**對第三方再授權 erpilot。每個終端使用組織必須**各自**取得自己的
授權（AGPL-3.0 / Small Business / Commercial）。

### 3.3 No Warranty / 無擔保

erpilot is provided "**AS IS**", without warranty of any kind, express
or implied, including but not limited to the warranties of
merchantability, fitness for a particular purpose, and noninfringement.

erpilot 以「**現狀**」提供，不附任何明示或默示擔保，包含但不限於商業性、
特定用途適用性、和不侵權之擔保。
...
During the Grace Period, You may continue using erpilot under this
license. After the Grace Period, continued use without (b) or migration
to (c) constitutes a license violation.

Grace Period 內你可繼續按本授權使用 erpilot。Grace Period 後若沒申請商業
...
Upon termination, You must either (a) cease all use of erpilot, or
(b) transition to AGPL-3.0 or Commercial terms.

授權終止後，你必須(a)停止所有 erpilot 使用，或(b)轉換到 AGPL-3.0 或
商業授權條款。
...
- GitHub Issue: https://github.com/fanchanyu/erpilot/issues (label `legal/small-business`)
- Email: *(to be filled in by maintainer)*

---

*This license is custom-drafted for erpilot's tri-license model and
...
erpilot's strategic differentiator for Taiwan SMB market.*

*本授權專為 erpilot 三軌制設計，參考 Elastic License v2 的「第三方使用限制」
條款、Sentry FSL、和 BSL 模式 — 但 ≤20 concurrent user 免費 tier 是
erpilot 為 Taiwan SMB 市場專屬設計的戰略差異化。*
