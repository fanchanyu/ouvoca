# Contributing to erpilot

歡迎貢獻！這份文件幫你快速上手 + 走完 CLA 簽署流程。

> 🌐 **Bilingual notice**: erpilot uses 繁體中文 for in-project communication.
...
```bash
gh repo fork fanchanyu/erpilot --clone
cd erpilot
...
erpilot 採 **AGPL-3.0 + 商業授權** 雙軌制（dual-license）。為了讓商業授權
...
1. **每個 commit 加 `Signed-off-by:` trailer**（用 `git commit -s`）
   - email 要是你自己掌控的
   - CI 會擋掉沒簽的 commit
2. **第一次貢獻**：開一個 [CLA 確認 issue](https://github.com/fanchanyu/erpilot/issues/new?template=cla-acknowledgement.yml)
   讓我們留紀錄
