...
| 文件 | 文件上寫的 | 程式實際 | 影響 |
|---|---|---|---|
| `PERMISSION_MODEL.md` | 約 95 個權限碼 | **231 個**（182 + 49 補充碼），角色 10 個正確 | 權限設定時會找不到項目；設計說明本身仍然正確 |
| `AGENT_CATALOG_ZH/EN.md` | 逐一列舉的是早期 26 個工具 | **201 個 / 19 領域**（文件開頭已補上正確總數與領域分佈，但未逐一列完） | 想看完整工具清單要翻 `backend/app/agents/domains/` |
