...
Ouvoca v3.36 print / export features cover:
...
| Signature validity | Ouvoca PDF carries **no e-signature**; use a separate signing service (TWCA / SecureSign / DocuSign) |
| Amount accuracy | Customer must manually verify amount, tax, validity period before sending |
| Attachments | Spec sheets, warranty, payment terms are **not** auto-attached by Ouvoca |
...
| No version history | Ouvoca PDF carries **no revision history**; reprint and recall old copies after corrections |
...
To the maximum extent permitted by applicable law, Ouvoca is **not liable** for disputes arising from:
...
| Internal cross-department | Use Ouvoca's **permission control** to query directly; avoid repeated downloads |
...
Ouvoca's `seed_demo_data_with_confirm` has built-in protection: **auto-skips when DB has data**.
...
- Ouvoca's built-in PDF template is generated by Ouvoca's own reportlab code
...
Customers using these fonts in commercial environments must comply with the respective vendor's license. Ouvoca does **not** embed or redistribute these font files.
...
- Ouvoca's ConfirmCard and LLM tool calls have **audit logs**
...
Ouvoca does **not** auto-archive PDFs; customers must store them (recommendation: monthly export + cloud backup + encryption).
...
**1. Printing**
Ouvoca is **not liable** for the **legal effect, accounting accuracy, or tax compliance** of PDF content. Customer must verify and have authorized personnel sign before external transmission.

**2. Export**
Ouvoca is **not liable** for the **storage, transmission, or third-party processing** of exported files. Customer must manage controls per PDPA, Trade Secrets Act, and contractual obligations.
...
Before adopting Ouvoca v3.36 print / export features, please confirm:
