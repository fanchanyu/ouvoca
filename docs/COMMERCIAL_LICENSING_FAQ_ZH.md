...
> 真正下決定前請和你的法務 / 律師確認，或開 [Commercial License Inquiry issue](https://github.com/fanchanyu/ouvoca/issues/new?template=commercial-license-inquiry.yml) 給維護者。
...
這是 Ouvoca 對 Taiwan SMB 的戰略差異化承諾。

> ⚠️ **建議閱讀**：免費「含 connector」指的是 **Ouvoca 不收技術授權費**。
> 要把 connector 接到您**現有商用 ERP**（例如 Workflow / ChengHang / SAP B1
> 等廠商之產品），各廠商之授權合約規定可能不同；具體請依貴司與該廠商之合約
> 為準。建議客戶於啟用前先和原 ERP 廠商書面確認授權範圍。Ouvoca **不參與、
...
舉例：
- 公司 30 人有 Ouvoca 帳號，但只有 18 人**正在用**（過去 15 分鐘有動作）→ ✅ 適格
- 公司 15 人有帳號，全部都在 dashboard 上看著（即使沒點任何東西）→ ✅ 適格（15 ≤ 20）
- 公司 50 人帳號，平常都用，但峰值（如午休前 11:45）有 25 人同時動作 → ❌ 不適格

實務上：用 Ouvoca 內建 analytics dashboard 看「peak concurrent users」就知道。
...
- 你**修改**了 Ouvoca 然後**散布給多個客戶** → 不適格，需要商業授權
...
### Q2: 我們公司想內部用 Ouvoca，要付錢嗎？

**不用。** 公司內部使用 = AGPL-3.0 沒有觸發「散布」條款。

⚠️ **但有兩個邊界**：
1. **多公司集團**：如果 Ouvoca 改動後給「**法律上不同的公司**」用（即使同集團），
...
### Q4: 我們是 SI（系統整合商），幫客戶導入 Ouvoca，要付錢嗎？
...
### Q5: 我把 Ouvoca 包成 SaaS，給好幾家客戶用，要付錢嗎？
...
1. 開 [Commercial License Inquiry issue](https://github.com/fanchanyu/ouvoca/issues/new?template=commercial-license-inquiry.yml) 或寄 email
...
- 啟動商業 SLA 支援 channel
- 顯示「Powered by Ouvoca Enterprise」品牌
...
- **多租戶配額**：自動降到單租戶
- **SLA 支援**：終止
- **品牌標示**：你必須在 30 天內移除「Powered by Ouvoca Enterprise」字樣
- **既有資料**：完全你的，不會被鎖
...
實務上：
- 你架了一個 Ouvoca SaaS 給客戶用 → 客戶有權向你索取 source
- 你內網架了 Ouvoca 給員工用 → 員工有權向你索取 source
...
### Q13: Ouvoca 的 LICENSE 檔可以放進我的商業產品嗎？

只放 LICENSE 檔 = 沒用。AGPL-3.0 的義務不會因為你保留 license 檔就消失。
你還是要：
- 揭露使用 Ouvoca
- 提供 Ouvoca 原始碼取得方式
- 你的 derivative work（基於 Ouvoca 改動的部分）也要 AGPL-3.0
...
### Q14: 我可以寫 plugin 接 Ouvoca 嗎？plugin 要不要 AGPL？

看 plugin 的「**結合方式**」：

- **API 呼叫**（HTTP / REST）：你的 plugin **不一定**要 AGPL，可以是任何授權。
  這是 AGPL 對「程式」邊界的解讀。
- **import / link**（Python `from Ouvoca import ...`、frontend `import 'Ouvoca/...'`）：
  你的 plugin 必須 AGPL-3.0（或相容授權）。

實務上：
- 寫 connector 用 HTTP/SSE 呼叫 Ouvoca → 可以閉源
- 寫 Python module 直接 import Ouvoca internals → 必須 AGPL

如果你想 import 但保有閉源權 → 商業授權。

### Q15: 我可以基於 Ouvoca fork 一份做自己的產品嗎？

**可以，但 AGPL-3.0 條款還是適用整個 fork**：

- 你的 fork 仍是 AGPL-3.0
- 任何客戶 / 使用者都可以向你索取 source
- 你要保留 Ouvoca 的 copyright notice
