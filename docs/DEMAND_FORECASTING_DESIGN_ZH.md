...
> **本檔性質**：跨**時間序列分析 / 機器學習 / ERP / AI** 四域之方法論文件，描述 Ouvoca v3.29 需求預測引擎之設計、實作與驗證。本版本接通 v3.25.10 → v3.28 規劃鏈之上游缺口：把「客戶手動輸入 MPS」改為「**由歷史銷售自動預測 + LLM 增強**」。
...
- v3.27 之 explainability：「為什麼下週要這麼多 M6 螺絲？」最終總是回到 MPS，**而 MPS 是客戶自己填的，Ouvoca 沒能力反問「你這個數字準嗎？」**
...
Ouvoca 預設：$k=3, \theta=2.0$（最近 3 期至少 2 期超出 2σ）。
...
**作者**：Ouvoca 工程團隊（含時序分析 / ML / ERP / AI 跨域學術方法論引用）
**版本**：1.0
