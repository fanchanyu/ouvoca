...
- 若因員工疏失（未鎖屏）導致誤操作，erpilot **不承擔責任**；客戶可依公司管理規章追究
...
- 預設由 `ERPILOT_BACKUP_DIR` 環境變數可改路徑
...
- erpilot 提供**功能介面**讓使用者觸發備份
- erpilot **不**自動排程備份（v3.x 路線圖）
- erpilot **不**自動異地備份
- erpilot **不**驗證備份完整性 — 客戶須自行 `sqlite3 file.db .schema` 確認可讀
...
- 此數據由 erpilot 之 governance.py tracker 在每次 LLM call 後寫入

### 4.2 限制

- erpilot 內部紀錄**可能與雲端廠商實際對帳單有出入**（model 升級、bug、tracker 漏記）
...
**1. TTL 延長**
erpilot 將 ConfirmCard TTL 由 5 分鐘改為 30 分鐘為**功能調整**；客戶須自行加強座位鎖屏 / 自動登出之配套；erpilot **不承擔**因 TTL 延長導致之誤確認損失。

**2. Undo**
erpilot 之 `undo_last_admin_change` 為**便利性功能**；**不**取代正式版本歷史 / 稽核回溯；超過 90 秒即失效；erpilot **不承擔**客戶因依賴 undo 而疏於慎重操作所致之損失。

**3. 備份**
備份為**客戶自為**之動作；erpilot 提供工具但**不**承擔備份檔之保管、加密、外流、毀損責任；客戶須依個資法、營業秘密法、商業會計法妥善處理。

**4. AI 成本**
`query_ai_cost_*` 為**內部估算**；**不**取代雲端 API 廠商之正式對帳；客戶於月底結算應以**廠商正式發票**為準。

**5. Disambiguation**
候選清單僅為**搜尋結果**；erpilot **不**保證資料庫中之客戶為「合法、現存、有效」之商業實體；客戶須自行確認交易對手身分。
...
### 9.3 技術配置
- [ ] `ERPILOT_BACKUP_DIR` 環境變數已設於本機儲存（不要在 Docker volume 內讓 docker compose down 清掉）
- [ ] 已測試「備份 → 模擬還原」之完整流程
